#!/usr/bin/python
# SPECIAL VERSION FOR 80 x 24 TERMINALS (v.2.1)
# ----------------------------------------------------
# MP3View v.2.1.
...
#
# 2.1 - 2001-04-27 - Special version of MP3View to work
# with standard sized terminal windows (80 x 24).
# Playlist dimensions can now be set as variables.
# Also, add handling for common exceptions when loading
# playlists.
#
# 2004-06-01 - Update contact info and website info for the
# project.
#
...
#
# Program notes:
# This script is designed for use with an 80 col, 24 
# line terminal.
# MP3 names are displayed as 74 chars,
...
# Look for the comment "CHANGE FOR YOUR ENVIRONMENT" below
...
# Set these to the height and width of your terminal
# (in characters)
...
playlist_height = playlist_height - 1

# The block_start and block_end variables define
# the first and last songs to display as a 'page'
# of songs on the playlist display 

block_start = 0
block_end = playlist_height 
cursor_pos = 0
a = [] # the playlist
...
# MP3 player location and args
mp3_loc = "/usr/bin/mpg123 -q "

# Mixer (volume control) location and args
aumix_loc = "/usr/bin/aumix -v "

# WAV file player location and args 
# (needed if you set sound cues on above)
dreamplay_loc = "/usr/bin/dreamplay "

# Location of sound cue files
soundfile_loc = "/home/rupe/car-sounds/"

# --- END OF PROGRAM MAPPINGS -----
...
def draw_screen(cursor_pos, block_start, block_end):
	bs = block_start
	print "CyMP3 2.1" + "-"*(playlist_width - 9)
	while (bs != block_end):
		...
		if (len(filename) > (playlist_width - 6)):
			filename = filename[:(playlist_width - 7)] + "~"
		if (bs == cursor_pos):
	...
	if (diff < (playlist_height -2)):
		while (diff < (playlist_height-2)):
			print "|  "
			diff = diff + 1	
	# Try to handle the length of the bottom window 
	# status line a little more
	# intelligently.
	status_string = str(cursor_pos) + "/" + str(len(a))+ " songs vol: " + str(c_vol)+ " "
	ss_len = len(status_string)
	print status_string + "-"*(playlist_width - ss_len)
	...
	while 1:
		...
		if (c != ''):
			...
			if (c == '1'):
				...
				move_inc = (playlist_height - 2) 
				# If we're less than a screen away
				# from the bottom, just go there.
				if (len(a) - block_end < (playlist_height -2)):
			...
			if (c == '2'):
				...
				# just go there.
				if (block_start < (playlist_height-2)):
...
# If we have a ListError, the user probably forgot to
# provide us with a playlist.  Print a helpful message.

except IndexError:
	print '\n\nPlease specify a playlist to use.'
	print 'Example: python mp3view2-console.py playlist.txt\n'
	# In the next version, we'll say this text.
	sndplay("problem")
	sys.exit(1)

# The user might also hand us a filename that doesn't exist.

except IOError:
	print '\n\nSorry, I couldn\'t find a playlist file by that name.'
	print 'Please check the path and spelling of the playlist file.\n'
	# In the next version, we'll say this text.
	sndplay("problem")
	sys.exit(1)
...
if (block_end > (playlist_height -2)):
	block_end = (playlist_height -2) 
